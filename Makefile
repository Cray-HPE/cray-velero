...
lint:
	CMD="lint charts/cray-velero" $(MAKE) helm

dep-up:
	CMD="dep up charts/cray-velero" $(MAKE) helm
...
ifdef CHART_VERSIONS
	CMD="package charts/cray-velero --version $(word 1, $(CHART_VERSIONS)) -d packages" $(MAKE) helm
else
...
images:
	CHART=charts/cray-velero $(MAKE) -s extracted-images annotated-images | sort -u
